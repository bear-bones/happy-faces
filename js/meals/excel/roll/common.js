...
function generate(done, blank, supper, period) {
   ...
   function execute(data, dates, filter_dates) {
      try {
         log.debug('Running /meals/excel/roll/common.js');
         log.debug(' -- meals.model.report_date: ', meals.model.report_date);
         log.debug(' -- dates: ', dates);
         log.debug(' -- filter_dates: ', filter_dates);
   ...
   try {
      ...
      case 'week':
         ...
         if (blank) {
            ...
            log.debug(start, end);
            log.debug(filter_start, filter_end);
         }
         break;
      case 'month':
         start.setDate(1);
      ...
      if (blank) {
         source = {
            data: meals.model.data, report_date: filter_end, no_punch_out: []
         };
         co(function* () {
...
function make_roll_page(data, ws, date, filter_date, room, blank) {
   ...
   ws['!merges'].push({s: {c: 0, r: ws.rows + 2}, e: {c: 15, r: ws.rows + 2}});
   ws['!merges'].push({s: {c: 0, r: ws.rows + 3}, e: {c: 15, r: ws.rows + 3}});
